...
test("deriveTaskTitle strips role tag and trailing punctuation", () => {
  assert.equal(deriveTaskTitle("@researcher сравни конкурентов в Варне?"), "сравни конкурентов в Варне");
});
...
test("intake plan resolves same-topic orchestrator direct answer without role tag", () => {
  const plan = buildTelegramIntakePlan(
    {
...
test("intake plan keeps complex founder question inside orchestrator front door", () => {
  const plan = buildTelegramIntakePlan(
    {
      text:
  ...
  assert.equal(plan.should_persist, true);
  assert.equal(plan.route.topic_role, "orchestrator");
  assert.equal(plan.route.resolved_role, "orchestrator");
  assert.equal(plan.route.route_source, "topic");
  assert.equal(plan.route.interaction_type, "direct_answer");
  assert.equal(plan.task, null);
...
test("intake plan resolves same-topic orchestrator task without role tag", () => {
  const plan = buildTelegramIntakePlan(
    {
  ...
  assert.equal(plan.route.resolved_role, "orchestrator");
  assert.equal(plan.route.route_source, "topic");
  assert.equal(plan.route.interaction_type, "multi_role_task");
  assert.equal(plan.task.id, "task_2");
